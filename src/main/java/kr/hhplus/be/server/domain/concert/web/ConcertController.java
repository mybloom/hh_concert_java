...
public class ConcertController {
    ...
    public ResponseEntity<BusinessApiResponse<List<PerformResponse>>> retrieveConcertPerforms(@RequestParam(required = true) long concertId) {
        if (concertId <= 0) {
            return ResponseEntity
                .badRequest()
                .body(BusinessApiResponse.failure(HttpStatus.BAD_REQUEST.toString(),"Invalid concertId"));
        }
        ...
        return ResponseEntity.ok().body(BusinessApiResponse.success(concertList));
    }
}
